...
RoleFit Workbench는 Next.js + TypeScript 기반 생산기술연구소 채용 검토 보조 도구입니다.

직무기술서, 추가 설명자료, 지원자 CV, 보조지표를 입력받아 담당부서 검토용 한국어 매칭 리포트를 생성합니다. 현재 버전은 `Local / Mock Auth / Synthetic data only` 기준이며 실제 SSO, Knox, DB, 운영 저장소는 아직 연결하지 않았습니다.
...
- [최종 개발 인수인계서](docs/current-development-handoff.md)
...
- 현업부서 검토 요청 Mock 흐름
- 별도 부서장 검토 포털
- 인터뷰 결과 임시저장·제출과 revision 충돌 검사
...
초딩 버전으로 말하면, 지금 앱은 “연습용 리모컨”이 달린 상태입니다. 회사 API라는 “진짜 리모컨”은 나중에 옆에 추가해야 합니다. 연습용 리모컨을 버리면 테스트와 장애 원인 확인이 어려워집니다.
